...
function OpportunityDetailsPage() {
    ...
    useEffect(() => {
        ...
        const options = {
            chart: { type: 'area', height: 220, toolbar: { show: false }, sparkline: { enabled: true } },
            stroke: { curve: 'smooth', width: 2 },
            fill: { type: 'gradient', gradient: { shadeIntensity: 1, opacityFrom: 0.5, opacityTo: 0.1, stops: [0, 90, 100] } },
            series: [{ name: 'Applications', data: opportunity.chart.series }],
            xaxis: { categories: opportunity.chart.labels, labels: { show: true, style: { colors: '#8A92A6' } }, axisBorder: { show: false }, axisTicks: { show: false } },
            yaxis: { show: false },
            grid: { show: false, padding: { left: 0, right: 0 } },
            colors: [customChartColor],
            tooltip: { x: { format: 'dd MMM' }, y: { formatter: (val) => `${val} applications` }, theme: 'dark' },
        };
