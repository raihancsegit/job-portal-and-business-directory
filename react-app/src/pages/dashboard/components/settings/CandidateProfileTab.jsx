...
const CandidateProfileTab = ({ showNotice, notice }) => {
    ...
    const token = localStorage.getItem('authToken');
     ...
     const handleCvFileUpload = async (e, index) => {
        ...
        setUploading(index); // আপলোডিং state সেট করা
        ...
        try {
            ...
            // state-এ ফাইলের URL এবং ID সেভ করা
            ...
            setUploading(null); // আপলোডিং state রিসেট করা
        }
    };
    ...
    const handleRepeaterChange = (e, index, section) => {
     ...
     // ======================================================
    // FIX #1: Add logic for 'cvs' to the handleAddItem function
    // ======================================================
    const handleAddItem = (section) => {
        if (section === 'education') {
            setProfile(prev => ({ ...prev, education: [...prev.education, { institution: '', degree: '', startYear: '', endYear: '', description: '' }] }));
        }
        if (section === 'experience') {
            setProfile(prev => ({ ...prev, experience: [...prev.experience, { title: '', company: '', startYear: '', endYear: '', location: '', description: '' }] }));
        }
        if (section === 'cvs') {
            if (profile.cvs.length < 4) {
                setProfile(prev => ({ ...prev, cvs: [...prev.cvs, { name: '', file_url: '', attachment_id: '' }] }));
            }
        }
    };
    ...
    return (
        <div className="candidate-profile">
            ...
            {notice && notice.type === 'candidate' && (
                <div className={`alert alert-${notice.status} mb-3`}>
                    {notice.message}
                </div>
            )}
            ...
            <form onSubmit={handleSave}>
                ...
                <div className="form-block">
                     ...
                     <input 
                        type="text"
                        className="form-control" 
                        placeholder="Enter skills, separated by commas (e.g., React, PHP, CSS)"
            ...
            <div className="form-block">
                <div className="d-flex justify-content-between align-items-center mb-3">
                    ...
                    {Array.isArray(profile.cvs) && profile.cvs.length < 4 && (
                        <button 
                            type="button" 
                            className="i-btn btn--sm btn--primary" 
                            onClick={() => handleAddItem('cvs')}
                        >
                            + Add CV
                        </button>
                    )}
                ...
                {Array.isArray(profile.cvs) && profile.cvs.map((cv, index) => (
                    <div key={index} className="repeater-item-box mb-3">
                        <div className="row g-3 align-items-center">
                            <div className="col-md-5">
                                ...
                                <input 
                                    ...
                                    value={cv.name || ''} 
                                    onChange={e => handleRepeaterChange(e, index, 'cvs')} 
                                    className="form-control" 
                                />
                            </div>
                            <div className="col-md-5">
                                ...
                                <input 
                                    type="file" 
                                    className="form-control" 
                                    onChange={e => handleCvFileUpload(e, index)}
                                />
                                {uploading === index && <small className="text-muted d-block mt-1">Uploading...</small>}
                            </div>
                            <div className="col-md-2 d-flex justify-content-end">
                                {profile.cvs.length > 1 && 
                                    <button 
                                        type="button" 
                                        className="btn-icon-danger mt-4" 
                                        onClick={() => handleRemoveItem(index, 'cvs')}
                                    >
                        ...
                        {cv.file_url && (
                            <div className="mt-2">
                                <small>
                                    Current file: <a href={cv.file_url} target="_blank" rel="noopener noreferrer">{cv.file_url.split('/').pop()}</a>
                                </small>
                            </div>
                        )}
                    </div>
